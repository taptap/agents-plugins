...
- 输出产物：
  - Story 场景：`change_analysis.json`、`code_change_analysis.md`、`test_coverage_report.md`、`change_coverage_report.json`、可选 `change_supplementary_cases.json`
  - Bug 场景：`change_analysis.json`、`code_change_analysis.md`、`change_fix_analysis.json`、`bug_risk_assessment.json`
- 失败门控：关联代码变更（MR/PR/diff）不能为空；无法确认的信息必须标记为"推测"或"待确认"
- 执行步骤：Story 7 阶段 / Bug 5 阶段，详见 [PHASES.md](PHASES.md)

## 意图识别

根据用户输入确定执行路径。代码变更（MR/PR 链接 / 本地 diff / diff 文本）是**唯一硬性必填**（contract.yaml `input.any_of`），需求来源（Story/Bug 链接、本地文档）是可选 — 缺失时降级，不阻断。

| 输入 | 动作 | 降级处理 |
|------|------|---------|
| Story 链接 / 需求文档 + 代码变更 | → Story 场景（7 阶段） | — |
| Bug 链接 / Bug 描述 + 代码变更 | → Bug 场景（5 阶段） | — |
| 仅代码变更，无需求来源 | → 降级为纯代码变更分析（按 Story 场景骨架，跳过需求相关步骤）| ✅ 不询问，直接降级；在 `change_analysis.json` 中标注 `scenario: "story_no_req"` |
| 仅 Story/Bug 链接，无代码变更 | → 调 `search_mrs.py` / `search_prs.py` 自动搜索；仍为空则停止报错 | ❌ 代码变更是硬性必填 |
| 模糊输入（连场景都不明） | → 询问用户提供至少一项代码变更来源 | — |
...
**下游字段契约**：requirement-traceability、metersphere-sync、ai-case 后端 `extract_story_output`、飞书卡片如何消费本 skill 产物，以及哪些字段是强依赖（动则触发下游故障），详见 [下游消费契约说明](references/downstream-contract.md)。
...
置信度量化评分见 [CONVENTIONS](../commons/CONVENTIONS.md#量化置信度评分)。

## 交叉验证（可选）

当运行环境支持并行 Agent 且变更文件 > 3 个时，阶段 3-5 可启用交叉验证模式（详见 [AGENT_PROTOCOL — 交叉验证协议](../commons/AGENT_PROTOCOL.md#交叉验证协议)）：
...
| codex-change-analyzer Agent（PHASES 3.5 cross-validation） | Opus | 通过 Codex CLI / OpenAI API 提供独立交叉视角，与主 Agent 同等深度 |
| 测试覆盖评估 | Sonnet | 双向比对 + 缺口风险排序 + 跨文档引用整理（结构化任务为主） |
...
当本次 diff 涉及**前端 UI 文件**且有 Figma 链接时使用，对照确认 UI 改动是否与设计稿一致、是否引入设计稿之外的隐式改动。按分级协议获取，详见 [test-shared-tools/SKILL.md](../test-shared-tools/SKILL.md#figma-设计稿获取)。

**前端 UI 文件判定**：路径或扩展名命中以下任一即视为前端变更：
- iOS：`.swift`、`.m`、`.mm`、`.xib`、`.storyboard`、路径含 `View/` `UI/` `Controller/`
- Android：`.kt`、`.java`、`.xml`（layout/drawable/values 目录下）、路径含 `ui/` `activity/` `fragment/`
...
| 模块 | 触发方式 | 文档 |
| ---- | -------- | ---- |
| Android 三方交互外部影响评估 | **阶段 3A.1 强制执行命中检测**，命中时自动激活 | [EXTERNAL-IMPACT.md](EXTERNAL-IMPACT.md) |
| Urhox 二进制影响分析 | **阶段 2.3a 强制筛选**，命中时执行 3A.2 | PHASES §3A.2 |

> ⚠️ **Android MR 必须执行命中检测**：凡仓库名/路径含 `android` 的 MR，在阶段 3A.1 必须将所有变更文件路径与 EXTERNAL-IMPACT.md「命中判断规则」表逐条比对，并将检测结果（命中/未命中）写入 `change_checklist.md` A section。**不允许静默跳过**。

命中后在 `code_change_analysis.md` 追加外部影响评估章节、在 `test_coverage_report.md` 追加外部影响测试访问评估章节，并将新增用例合并写入 `change_supplementary_cases.json`。

### 平台覆盖现状（盲区告知）

**当前外部影响评估仅覆盖 Android 平台**。iOS 与 Server（cps 等）虽然同样存在 SDK 交互、跨服务调用等外部影响风险，但本 skill 暂无对应的命中规则表 → **AI 分析会静默跳过这两个平台的外部影响评估**。

处理方式：
- 遇到 iOS / Server MR 时，AI 必须在 `change_checklist.md` A section 追加一条 `[ ] 外部影响评估：本平台暂无自动命中规则，已跳过 → 建议人工补充对照 SDK 兼容性 / 下游服务影响`
- 在 `change_analysis.json` 的 `action_items` 追加一条 `请人工评估 {iOS / Server} 外部影响：本次 diff 未做自动评估`
- 这样虽然没有评估深度，但**盲区被显式标注、不会被悄悄忽略**

> 后续如果团队补齐 iOS / Server 的高风险文件清单，可创建 `EXTERNAL-IMPACT-ios.md` / `EXTERNAL-IMPACT-server.md` 并扩展本表。
...
| 6. generate | 为覆盖缺口生成补充用例 | `change_supplementary_cases.json` |
...
> **Android 三方交互命中时（按需）**：`code_change_analysis.md` 追加外部影响评估章节，`test_coverage_report.md` 追加外部影响测试访问评估章节，`change_supplementary_cases.json` 合并外部影响建议用例。不新增文件，详见 [EXTERNAL-IMPACT.md](EXTERNAL-IMPACT.md)。
...
| 5. output | 风险评估和最终产出 | `change_analysis.json`、`bug_risk_assessment.json` |
...
### change_supplementary_cases.json（Story 场景，可选）

使用 [CONVENTIONS.md 用例 JSON 格式](../commons/CONVENTIONS.md#用例-json-格式)。

### ca_summary.json（Story / Bug 均必做，供下游消费）

**为什么**：`change_analysis.json` 字段多结构散，下游工作流（test-case-generation / 前端摘要 / 飞书卡片）只关心"哪些是变更新增功能点 / 哪些模块要回归 / 风险等级"。约定一个稳定的窄摘要供平台后端 `extract_story_output` 消费，详见 [commons/contracts/ca-summary.schema.json](../commons/contracts/ca-summary.schema.json)。
...
**字段约束**：`changed_modules[*]` <= 60 字、`new_features[*].title` <= 80 字（注入下游 prompt 会被截断）；`risk_count` 是整数总数等于 `risk_breakdown` 三档相加；常见拼写错误黑名单 `modules` / `risks_count` / `features`（详见 [PHASES.md §7.x](PHASES.md#7x-ca_summaryjson-结构化摘要必做)）。
...
- [ ] `ca_summary.json` — 非空，供下游 TC 生成 / 前端摘要消费（schema 见 commons/contracts/ca-summary.schema.json）
- [ ] `change_supplementary_cases.json` — 条件必做，当 `coverage_rate < 80%` 或 `gap_count ≥ 3` 时必须生成；未达阈值时跳过并在 `test_coverage_report.md` 末尾留痕

**Bug 场景：**
- [ ] `change_analysis.json` — 非空，包含变更分类和影响面分析
- [ ] `code_change_analysis.md` — 非空，包含代码变更分析过程
- [ ] `change_fix_analysis.json` — 非空，包含修复完整性评估
- [ ] `ca_summary.json` — 非空，供下游消费
...
- 回读中间文件、中断恢复、脚本路径等通用约定见 [CONVENTIONS](../commons/CONVENTIONS.md)
- **Android MR 三方交互检测强制执行** — 阶段 3A.1 必须对 Android MR 执行命中检测并将结果写入 `change_checklist.md`，检测结果是后续阶段 4 和阶段 6 是否激活外部影响评估的依据
