...
**触发条件**：本次 diff 涉及前端 UI 文件且 prefetch 提供了设计稿链接（见 SKILL §可用工具.Figma 的「前端 UI 文件判定」）。纯后端变更跳过本节。

判定结果**必须**写入 `change_checklist.md` 的「前端变更命中检测」节，无论命中与否：
- **未命中**：写入"经检测，本次 diff 未涉及前端 UI 文件，跳过设计稿获取"
- **命中**：列出命中的前端文件路径，进入下面的获取步骤

获取步骤（命中时执行）：
- **Figma 链接**：按分级协议（`mcp__figma__get_metadata` → `get_design_context` → 按需 `get_screenshot`）拉取设计稿数据，保存到 `design_context.md`；如 prefetch 已注入 `figma_prefetch.reachable=false`，跳过调用并在 `design_context.md` 写入"设计稿不可访问，原因：{error}"
- **飞书链接**：使用 `fetch_feishu_doc.py`
- 无链接或不可访问则跳过获取步骤，但仍要保留「前端变更命中检测」节作为执行痕迹
...
### 2.3a 附加：Urhox 有效改动筛选（Urhox 项目强制执行）
...
写入 `change_checklist.md`。**文件结构强制分两个 section**，避免"AI 执行痕迹"和"人工评审"语义混在一起：

```markdown
# 变更分析清单

## A. 执行痕迹（AI 写入，可勾选 [x] 标记已分析）

> 本节由 AI 在各阶段顺序写入，体现可审计的执行过程：前端变更命中检测、Android 三方交互命中检测、Urhox 筛选、Codex 交叉验证启动状态、覆盖率计算等。AI 完成对应步骤后可勾选为 `[x]`，表示该步骤的执行已留痕。

- [ ] 阶段 2.2 前端变更命中检测（命中 / 未命中 + 命中文件列表）
- [ ] 阶段 3A.1 Android 三方交互命中检测（命中 / 未命中 + 命中模块）
...
后续阶段引用此清单逐项处理时，**只允许在 A section 勾选自己写入的执行痕迹项；B section 始终保持 `[ ]`**。
...
**3A.1 Android 三方交互命中检测（强制步骤，不可跳过）**
...
2. 后续：追加到对应章节
3. `change_checklist.md` **B section（人工评审）保持 `- [ ]`**——飞书文档面向 QA / 评审会，勾选权归读者；**不要**因为"AI 已分析完"就把 B section 中的项标 `[x]`。A section（执行痕迹）允许 AI 标 `[x]` 留痕。chat 输出进度即可
...
阶段 3 完成后，读取 `codex_change_findings.json`，按 [AGENT_PROTOCOL 交叉验证协议](../commons/AGENT_PROTOCOL.md#交叉验证协议) 合并：
...
分析维度：功能影响、数据影响、跨平台/跨服务影响、性能影响。

**前端变更场景必做：设计实现完整性分析**
- 触发条件：`change_checklist.md`「前端变更命中检测」节标记为命中，且 `design_context.md` 已生成（即 §2.2 成功获取到设计稿）
- 比对维度：
  - **diff 是否覆盖设计稿揭示的全部 UI 状态**（默认/空/loading/error/异常态）— 缺失则记入 §4.1 直接影响
  - **diff 是否引入设计稿之外的 UI 改动**（"超出设计范围"）— 记入 §4.3 高风险场景，并标注"待 PM/设计确认"
  - **UI 文案、交互、视觉规范一致性**（按钮文案、提示文案、错误文案）— 不一致记入 §4.1 直接影响
- 输出位置：在 `code_change_analysis.md` §4 影响面评估末尾追加「设计实现完整性」子节
- 设计稿不可访问时（`design_context.md` 记录失败原因）：跳过本分析；**不扣减 `risk_level`**（工具失败不等于变更风险增加，污染评分会让人类失去信号）；按以下方式留痕，让人类一眼看到这个缺口：
  - 在 `code_change_analysis.md` §4 末尾显式标注"因设计稿不可访问（{失败原因}），本次未进行设计实现完整性比对"
  - 在阶段 7 的 `change_analysis.json` 的 `key_findings` 追加一条 `"设计稿不可访问 — 前端 UI 改动未与设计稿做完整性比对，建议人工补查（原因：{失败原因}）"`
  - 在 `action_items` 追加 `"检查 FIGMA_API_KEY 配置或设计稿分享权限，或由 QA 手动对照设计稿评审本次 UI 改动"`
...
**触发条件（强制门控，二选一即触发）**：
- `coverage_rate < 80%`（来自阶段 5 §3 的精确公式 `已覆盖 N / 总变更点 M`），或
- `gap_count ≥ 3`（来自阶段 5 §2 列出的缺口数）

满足上面任一条件 → 必须生成 `change_supplementary_cases.json`，不允许"自觉无缺口跳过"。同时满足 `coverage_rate ≥ 80%` 且 `gap_count < 3` → 跳过本阶段，并在 `test_coverage_report.md` 末尾追加一行 `> 覆盖率 X%、缺口数 N，未达生成阈值，本次不生成补充用例` 作为执行痕迹。

**写入方式（强制）**：调用 MCP tool `mcp__cases__save_test_cases(file_path='<workdir>/change_supplementary_cases.json', cases=[...])`。**禁止用 Write 工具直接写 *_cases.json**，会被 hook 拒绝。该 tool 的 input_schema 已强约束字段（title/priority/preconditions/steps），违反会被 API 在生成阶段直接拒绝，无需手写 JSON 字符串。详见 [CONVENTIONS.md 用例 JSON 格式](../commons/CONVENTIONS.md#用例-json-格式)。
...
1. 生成 `change_analysis.json`（总结）
   - **特殊处理**：如果阶段 4「设计实现完整性」因设计稿不可访问被跳过，必须把缺口写入 `key_findings` + `action_items`（具体文案见 §4「设计稿不可访问时」），但 `risk_level` 不因此扣减
...
**为什么**：`change_analysis.json` 字段多结构散，下游工作流（TC 生成）只关心"哪些是变更新增功能点 / 哪些模块要回归 / 风险等级"。约定一个稳定的窄摘要供平台后端消费，详见 `../commons/contracts/ca-summary.schema.json`。
...
#### 2.3a 附加：Urhox 有效改动筛选

同 Story 场景 2.3a，Urhox 项目触发时执行。筛选完成后，该 PR 的代码变更范围由筛选结果替代——后续所有阶段（阶段 3 根因分析、阶段 4 影响面评估、阶段 5 输出）均只针对命中文件操作，PR 原始 diff 中其余改动不再纳入。

### 阶段 3: diff/context — 逐代码变更分析

同 Story 场景阶段 3（3A + 3A.1 Android 命中检测 + 3A.2 Urhox 二进制影响 + 3B），每个代码变更完成后追加写入 `code_change_analysis.md`。
...
2. 生成 `bug_risk_assessment.json`（残余风险评估和行动建议；带 bug_ 前缀以避免与 requirement-traceability 的 risk_assessment.json 撞名）：
