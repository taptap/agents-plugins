...
代码生成类 skill 在 verify 阶段尝试执行测试后，将执行结果持久化为 JSON 文件，供下游 test-failure-analyzer 消费。按 skill 区分文件名以避免撞名：unit-test-design 产出 `unit_test_execution_report.json`，integration-test-design 产出 `integration_test_execution_report.json`。两者结构一致。
