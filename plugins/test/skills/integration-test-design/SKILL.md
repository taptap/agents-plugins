...
以下场景 AI 无法可靠地确定断言预期值或断言方式，**不生成测试代码**，只在 integration_test_plan.md 中记录场景描述和跳过原因，由开发者自行补充。
...
**跳过时在 integration_test_plan.md 中记录**：

```markdown
| 场景描述 | 使用的 helper | 跳过原因 |
| --- | --- | --- |
| user 路由缺少 userId 的降级行为 | RouterTestHelper.assertLastVC | 不确定降级返回的类型是否经过 Router.route()，helper 可能拦截不到 |
```
...
- helper 的 install 方法明确说明了拦截点，且被测场景确定流经该拦截点
...
7. **出口断言扫描**（硬性出口条件）：在提交测试文件前，对每个 test 方法扫描是否包含至少 1 个断言调用（参见 [断言审计协议](../../CONVENTIONS.md#断言审计协议)）。发现零断言方法时，原地补充断言后再进入 verify 阶段。目的确实是"验证不崩溃"的方法必须标注 `[crashSafety]` 并添加最低限度断言
