...
2. 执行连通性检查：
   ...
   ```
   - 返回 `status: ok` → 继续
   - 缺少 MS 环境变量 → **停止**，引导用户打开飞书 [MeterSphere 配置 (.env)](https://xd.feishu.cn/wiki/K4Cxw8HE5itR16kFFYicSctAnrc)，把配置块整段粘给 AI 写入 `test-shared-tools/scripts/.env`
   - 其他失败 → **停止**，输出错误信息（helper 失败 stderr 已是结构化 JSON，参见 SKILL.md「Helper Commands」节）

### 1.1 输入路由

按 [CONVENTIONS.md](../commons/CONVENTIONS.md#上游输入消费) 定义的优先级确认输入：

1. 公共工作区 `test_cases/final_cases.json` 存在 → 作为主输入
2. 兼容旧版：当前工作目录中存在 `final_cases.json` → 作为主输入，但新产物仍写入公共工作区 `metersphere/`
3. 工作目录中存在补充用例文件（任一）：`change_supplementary_cases.json` / `review_supplementary_cases.json` / `supplementary_cases.json` → 作为主输入。多个并存时按列出顺序优先（按上游 skill 区分文件名以避免撞名；`supplementary_cases.json` 仅 test-case-generation 内部使用）
3. 以上均不存在 → **停止**

### 1.2 确定执行模式

- 用户指定 `mode=execute` 且公共工作区 `traceability/<change_set_slug>/forward_verification.json` 存在，或显式传入旧版 `forward_verification.json` → execute 模式
- 否则 → sync 模式
...
| mode | precondition |
| --- | --- |
| sync | `test_cases/final_cases.json` 存在（旧版可兼容显式传入 `final_cases.json`）；schema 合法（顶层 array、每条满足 ../commons/CONVENTIONS.md#用例-json-格式）；`case_id` 稳定存在；`module` 缺失/`null`/空字符串已兜底为 `未分类`；同 `(module, title)` 不重名 |
| execute | `traceability/<change_set_slug>/forward_verification.json` 存在；通过 `validate-fv`；`metersphere/ms_case_mapping.json` 存在；mapping 的 `source_cases_file.sha256` 与 `test_cases/final_cases.json` 一致 |
...
python3 $SKILLS_ROOT/test-shared-tools/scripts/metersphere_helper.py \
  ...
  [--mapping-out $TEST_WORKSPACE/metersphere/ms_case_mapping.json] \
  [--report-out $TEST_WORKSPACE/metersphere/ms_import_report.json]
```

- `--requirement`（推荐）：指定需求名称，会先在 `parent_module_id` 下创建需求级父模块，子模块再挂在其下。结构为：`父模块 / 需求名 / 子模块A, 子模块B...`
- `--mapping-out`：mapping 落盘路径；公共工作区中必须指定为 `metersphere/ms_case_mapping.json`
- `--report-out`：import report 落盘路径；公共工作区中必须指定为 `metersphere/ms_import_report.json`。若省略且指定了 `--mapping-out`，helper 默认跟随 mapping 同目录
- 不指定 `--requirement` 时，子模块直接挂在 `parent_module_id` 下

该命令自动：
- 按用例的 `module` 字段分组；缺失、`null` 或空字符串统一按 `未分类` 处理
...
# 自动清理 stale 条目
python3 $HELPER refresh-mapping --mapping-path PATH --cases-path $TEST_WORKSPACE/test_cases/final_cases.json --apply
```
...
```bash
python3 $HELPER rebuild-mapping --plan-id <new_plan_id> --cases-path $TEST_WORKSPACE/test_cases/final_cases.json \
  [--mapping-out PATH]
```
...
从 `metersphere/ms_case_mapping.json` 的 `entries[].ms_id` 提取所有 ms_id，关联到计划：
...
将计划信息保存为 `metersphere/ms_plan_info.json`：
...
此阶段仅在 `mode=execute` 时执行，需要 `traceability/<change_set_slug>/forward_verification.json` 输入（来自 requirement-traceability，正向用例中介验证结果）。

> 直接消费 traceability 产出的 `forward_verification.json` 作为执行结果，不引入独立的中间用例文件层。

### 4.0 冒烟测试报告边界

`smoke_test_report.json` 的 verdict 五档枚举以 `../commons/contracts/smoke-test-report.schema.json` 为准：
`pass` / `fail` / `pass-with-degraded-input` / `fail-with-degraded-input` / `inconclusive`。

当前 `writeback-from-fv` 只消费 `forward_verification.json`，不直接读取 `smoke_test_report.json`。
smoke-test 模式在 requirement-traceability 中只产出报告，不写回 MS，避免把冒烟级全局结论误写成逐用例执行结果。
如外部编排需要按 smoke verdict 做全量拦截，应在调用 execute 前处理。
...
python3 $SKILLS_ROOT/test-shared-tools/scripts/metersphere_helper.py \
  writeback-from-fv \
  --plan-id <plan_id> \
  --fv-path $TEST_WORKSPACE/traceability/<change_set_slug>/forward_verification.json \
...
- 测试计划：{ms_plan_info.plan_url}
- 用例库入口：{ms_import_report.metersphere_url}
```

其中 `测试计划` 是用户可点击进入执行计划的主入口，必须展示；如果 `ms_plan_info.json.plan_url` 缺失，不能声明同步完成，必须回到阶段 3 补齐计划信息。
...
- sync 模式：`metersphere/ms_case_mapping.json`（v2）、`metersphere/ms_plan_info.json`、`metersphere/ms_import_report.json` 已落盘，且最终回复已展示 `ms_plan_info.json.plan_url`
