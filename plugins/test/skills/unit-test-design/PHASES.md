...
6. **出口断言扫描**（硬性出口条件）：在提交测试文件前，对每个 test 方法扫描是否包含至少 1 个断言调用（参见 [断言审计协议](../_shared/ASSERTION_AUDIT.md)）。发现零断言方法时，原地补充断言后再进入 verify 阶段。仅验证"不崩溃"的方法必须标注 `[crashSafety]` 并添加最低限度断言
...
5. 5a 和 5b 的发现按 [交叉验证](../../CONVENTIONS.md#交叉验证) 合并
