...
4. **检测评审模式**：根据 prompt 顶部的模式标记确定信息基底（对齐 requirement-clarification 的 4 模式命名，去掉旧版『降级』贬义文案）：
...
**verdict 与 confidence 的硬规则**（违反时调整 verdict）：
- `not_ready`：confidence < 50 **或** 阻断项数 > 5
- `ready_with_conditions`：confidence ∈ [50, 79] **或** 阻断项数 ∈ [1, 5]
- `ready`：confidence ≥ 80 **且** 阻断项数 = 0
