...
4. 确认评审范围，在 chat 中输出基本信息（标题/文档名称）
...
**唯一阻断条件**：需求文档必须获取到；关联文档、设计稿、现有代码、负责人信息均为非阻断。

需求文档已获取 → 在 chat 中输出数据收集摘要后进入阶段 3。未获取 → 提示用户补充并等待。
  ...
  "questions": [
    {
      ...
      "header": "{维度简称}",
      "options": [
        {"label": "确认阻断", "description": "评审前/评审会上必须确认"},
        {"label": "降级为关注项", "description": "开发前确认即可"},
        {"label": "忽略", "description": "已知风险或不适用"}
      ],
      "multiSelect": false
    }
  ]
