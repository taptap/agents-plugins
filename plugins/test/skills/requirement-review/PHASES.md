...
后台/编排环境中，第 3 步的“确认”是非阻塞摘要：除非本轮确实来自用户补充信息并需要继续追问，否则不要停在此处等待自然语言回复。
...
后台/编排环境中，该“确认”只表示输出摘要供前端展示；除非触发阶段 2.6 的数据门控阻断或阶段 4.4 的 AskUserQuestion，否则必须继续进入 review 和 output 阶段。
...
当评审发现阻断级问题时，调用 AskUserQuestion 工具确认（格式见 [CONVENTIONS.md](../commons/CONVENTIONS.md#askuserquestion-交互式提问)）。
...
> **CRITICAL — 选项溯源**：本场景的 option 是固定处置选项（确认/降级/忽略），属于元操作。`evidence_tag` 标 `derived`，`evidence_ref` 必须包含 `review_checklist.md` 中对应阻断项的原文摘录（用 `「」` 圈出关键描述），让 AI 把阻断项原话敲出来作为自检。**不允许**在 option label/description 中编造未在阻断项原文出现的具体名词。详见 [输出溯源原则](../commons/CONVENTIONS.md#输出溯源原则)。
>
...
3. 各职能待确认问题列表（报告核心，按 PM / Dev / QA / Design 分组；问题维度名只用人类可读的，不写 FB2/LC2 等内部编码）。**严格按 [TEMPLATES.md §3](TEMPLATES.md) 双层结构**：父项 `- **{编号}（{维度}）**：{描述}` **不带 checkbox**，每个父项**必须**嵌套二选一 `- [ ] 有效问题，需补充更新：____` / `- [ ] 无效问题，原因：____`，缺一不可——飞书评审会需要这两个空 checkbox 来勾选决策；省略子项 = 报告丢失核心可勾选行动清单。
...
**为什么**：`report.md` 是给人看的长文，平台后端的下游工作流（TC 生成等）和前端摘要卡片需要**结构化短摘要**才能消费。详见 `../commons/contracts/rr-summary.schema.json`。
