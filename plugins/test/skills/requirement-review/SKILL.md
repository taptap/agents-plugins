...
handoffs:
  - label: 生成测试用例
    ...
    when: 评审结论为「ready」或「ready_with_conditions」，准备进入用例设计阶段
    prompt_hint: "基于本次产出的 review_checklist.md 和 rr_summary.json 生成用例；阻断项已清空或转为关注项后再启动"
    recommended: true
---
...
8. **反塌缩约束** — 检测「X 不…」「非 X」等否定句式（隐含 N 元枚举塌缩为 2 元，下游开发必漏 case）；检测多 tab/列表变体场景缺少差异化说明（类继承会让 UI 元素隐式扩散）。详见 PHASES.md 4.1.7 / 4.1.8
...
| 任务 | 推荐模型 | 理由 |
| --- | --- | --- |
| 需求文档解析和评审 | Opus | 需求理解是整个 pipeline 质量天花板；单 Agent 强推理模式（假设/反例/结论）对模型推理深度要求高 |
...
- [ ] `review_checklist.md` — 非空，包含 12 维度评审清单 + 「文档文案校对」区块 + 「分类变量枚举」区块（无发现也须显式写入）
- [ ] `report.md` — 非空，包含精简评审报告
- [ ] `rr_summary.json` — 非空，含 `verdict` / `issue_count` / `blocking_issues` / `confidence`（详见 PHASES.md 5.1.5）

全部必须项通过后，按 `verdict` 条件输出 Next Steps（见下）。如任一必须文件缺失，**停止并补生成**，不允许声明完成。

### Next Steps 输出（按 verdict 条件）

**verdict = `ready` 或 `ready_with_conditions`**：在 chat 末尾输出以下推荐（推荐项来源于 frontmatter `handoffs[recommended=true]`）：

````markdown
## ✓ 需求评审完成

- 评审结论：{ready / ready_with_conditions}
- 阻断项：N 个 / 关注项：M 个 / 待确认：K 个
- confidence：XX/100
- 输出文件：requirement_understanding.md / review_checklist.md / report.md / rr_summary.json

## 下一步

**推荐：生成测试用例** — 基于 review_checklist.md 和 rr_summary.json 设计测试用例
...
**verdict = `not_ready`**：不推荐 TC 生成接力，改为：
