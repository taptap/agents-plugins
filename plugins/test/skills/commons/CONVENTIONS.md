...
详见 [ASSERTION_AUDIT.md](ASSERTION_AUDIT.md)。
...
输出到飞书云文档（即 `report.md` 类经 `create_feishu_doc.py` import 的报告）的 markdown 必须遵守本节规则。**这是各 skill 的 TEMPLATES.md 唯一的渲染规则真源**——`关键约束` 章节应直接引用本节，不要在 skill 内部再写一份。

### 表格 vs Bullet 选择规则

经 2026-05 实测，飞书 import **已稳定支持** 4-6 列 markdown 表格（含中文长描述、特殊符号、数字），不再像旧版那样被拆成离散 text block。但表格不是万能——按以下场景判断：

**适合用表格**（需要扫读对比 / 横向比较 / 多列均匀长度）：
- 矩阵类（如需求覆盖矩阵：需求点 × 用例 × 状态 × 置信度）
- 等长字段清单（如风险摘要：编号 × 描述 × 优先级 × 影响 × 措施）
- 模型字段变更（模型名 × 字段 × 变更性质 × 影响）
- 影响域分析（功能域 × 描述 × 风险等级）

**保留 bullet 列表**（嵌套 / 层级 / 长描述主导 / 需要 checkbox）：
- 树状层级（如变更文件按业务域分组，下级为文件清单）
- KV 计数指标（覆盖率、用例数分布、问题数）
- 描述列是主要内容、其他列只是标签的（描述会撑爆表格）
- **需要原生 checkbox 交互**——飞书 import 不支持表格单元格内的 markdown checkbox `[ ]`，必须用列表项实现（如 `requirement-review` 第 3 章各职能问题列表）

灰色场景下偏向**先做表格试**：表格语义更清晰，扫读成本低；如果实测发现某列被挤变形，再退回 bullet。

### 禁用元素清单（飞书 import 已知不兼容）

- **emoji 状态符号** ✅/⚠️/❌：飞书 import 会把 ✅ 转 `[OK]`、⚠️ 转 `[!]`、❌ 触发 bitable block 解析导致整行内容丢失。状态符号统一用中文方括号标记（`[通过]`/`[有条件]`/`[不通过]`、`[高]`/`[中]`/`[低]` 等）
- **HTML `<details><summary>` 折叠**：飞书 import 不识别折叠 block，会把 summary 转为误导性文字"点击展开"但实际不能点
- **表格单元格内的 markdown checkbox `[ ]`**：渲染为纯文本无法点击，必须用列表项实现 checkbox 交互
- **不写 H1 标题**：飞书云文档名已包含报告类型（如「{需求名称}-需求评审-{YYYYMMDDHH}」），H1 会重复

### 优先级 / 状态术语

- 严重性 / 优先级二档 `阻断` / `关注`，工程化别名 `P0` / `P1`（详见 [REQUIREMENT_DIMENSIONS.md 术语映射](REQUIREMENT_DIMENSIONS.md#术语映射)）
...
详见 [AGENT_PROTOCOL.md](AGENT_PROTOCOL.md)。
...
详见 [TRACEABILITY_PROTOCOL.md](TRACEABILITY_PROTOCOL.md)。
