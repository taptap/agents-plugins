...
- **metersphere-sync (execute 模式)**：本 skill 的 **Phase 6 writeback** 会调用 metersphere-sync execute，消费 `forward_verification.json` 并回写 MS 测试计划用例的 Pass/Failure 状态。仅 `mode != "smoke-test"` 触发；冒烟测试模式不写 MS。独立调用本 skill（标准模式）即可完成回写，不需要再串联调用 metersphere-sync
...
- [ ] `forward_verification.json` — **必须非空**：Phase 3.2 正常产出 → 用例级；Phase 3.2 偏离 → Phase 4.6 必须从 coverage_report 兜底合成。**禁止以「降级路径」为由跳过**
- [ ] `ms_sync_report.json` — **当 `ms_plan_info.json` 存在时必须产出**：Phase 6 writeback 必须实际跑成功；如 MS 连通失败 → 报告中标注 `failed` 但仍须写文件，不允许静默跳过
