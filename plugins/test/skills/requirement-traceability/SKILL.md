...
当调用方提供 `design_link`（Figma）+ `code_dir`（前端代码目录或代码文件清单）时，正向通道额外执行 UI 还原度对比。使用 Figma MCP 获取结构化设计数据，从 `code_dir` 读取相关组件的样式定义，通过 Task 工具启动 `ui-fidelity-checker` Agent 做静态对比，输出 `ui_fidelity_report.json` 合并到 `traceability_coverage_report.json`。

**纯静态对比**（不依赖运行时浏览器），confidence 上限 60。触发条件：`design_link` + `code_dir` 都存在且 Figma MCP 可用。详见 PHASES.md §3.4。

### API 契约感知检查（条件触发）

当代码变更涉及 API 交互（网络请求路径、API 模型、请求参数等）时，§3.2.5 通过 Task 工具启动 `api-contract-validator` Agent（见 [agents/api-contract-validator.md](../../agents/api-contract-validator.md)），做完整的路径 / 参数 / 响应字段 / Breaking Change 4 维度校验。结果合并到 `traceability_coverage_report.json` 的 `api_contract` 字段。

触发条件（满足任一）：代码变更包含 API 相关文件、需求点涉及接口交互、同时存在前端和后端 MR/PR。

**与独立 skill 的关系**：`api-contract-validator` Agent 同时被独立的 `api-contract-validation` skill 使用——同一逻辑，两个调用入口：
...
| 正向降级（PHASES 3.2.4，主 agent 内联） | Opus | 仅在用例数据严重缺失时触发，从需求描述推断需求 → 代码模糊映射 |
...
| --- | --- | --- |
| 1 init（含 1.3 precondition）| ✓ 全跑（1.3.a + 1.3.b + 1.3.d） | ✓ 仅跳 1.3.a 中的 `mapping_sha` 校验项（不写 MS）；其余 1.3.a + 1.3.b + 1.3.d 全跑 |
| 2 fetch | ✓ | ✓ |
| 3 map | ✓ | ✓ |
| 4 output（4.1–4.5） | ✓ | ✓ |
...
> 1.3.d 把"用例输入完整性"做成单一权威字段 `input_quality`，下游 4.6 / 4.5 / 5S.2 都按此降级；smoke-test 模式不豁免（避免"无任何用例输入"还硬判 pass/fail）。详见 PHASES §1.3.d。

PHASES.md 内涉及 mode 分支的段落（5S.1 / 5S.2 / Phase 5 loop / Phase 6 writeback）仅以一句"见 SKILL.md mode dispatch 表"引用本表，**不重复列条件**，避免散落不一致。

## ID 体系（单一权威表，TEMPLATES / PHASES 中的所有示例须遵守）

| 字段 | 取值规范 | 备注 |
| --- | --- | --- |
| `requirement_id` | 优先使用上游 FP-N（来自 `clarified_requirements.json` / `requirement_points.json`）；独立使用本 skill（无上游）时才用本地 R-N；supplementary cases 语义匹配不到时归 `FP-UNMAPPED-{N}` | 主键，跨 skill 关联用 |
...
| `case_id`（**supplementary 来源**） | 直接复用 ca 的 `TC-{N}`（不重命名，下游引用稳定） | 由 §3.1 优先级 1.5 档消费 `change_supplementary_cases.json` 时产生，衍生 fv 条目额外标 `case_source: "supplementary"` |
| `case_id`（正向降级 / forward fallback） | `FORWARD-TRACER-{requirement_id}` | PHASES 3.2.4，名称沿用以保持下游兼容 |
| `case_id`（4.6 兜底合成） | `FORWARD-TRACER-FP-{N}` | PHASES 4.6（仅 input_quality == "low" 时触发） |
...
| `code_analysis.md` | map | 逐代码变更的分析记录（含「反向追溯输出（主 agent 内联）」节，由 3.3 写入） |
| `requirement_doc.md` | fetch（条件） | 在线获取的需求文档原文 |
| `ui_fidelity_report.json` | map 3.4（条件触发） | UI 还原度检查结果（design_link + code_dir 同时提供时由 §3.4 内置生成） |
...
- [ ] **当 `ms_plan_info.json` + `ms_case_mapping.json` 都就位时**（即上游已完整跑过 mode=sync）：
  ...
  - [ ] `pass_with_caveats.md` — 即使没有 caveat 条目也要落盘
  - [ ] `pending_external_validation.md` — 同上
- [ ] **当 `ms_plan_info.json` 或 `ms_case_mapping.json` 缺失时**（D10 graceful skip 路径）：
  - [ ] chat 摘要必须输出 `writeback_skipped: missing_ms_plan_info | missing_ms_case_mapping` 标记
  - [ ] 不要求产出 ms_sync_report / enriched fv / caveats md（Phase 6 整段被跳过）
...
- **硬阻断条件**（任一不满足即 STOP）：代码变更非空；标准模式额外要求 final_cases.json 存在 + mapping sha 一致（如 mapping 存在）。详见 PHASES 1.3.a
- **软警告条件**（缺失不阻断，Phase 6 优雅 skip）：mapping / plan_info 不存在。详见 PHASES 1.3.b
