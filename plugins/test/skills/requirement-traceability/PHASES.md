...
本 skill 使用两个目录变量，后续 `$TEST_WORKSPACE` 均指公共需求工作区根目录：

- `$TEST_WORKSPACE`：公共需求工作区根目录，即 `requirement_<stable_id>/`
- `$TRACEABILITY_RUN_DIR`：本次回溯运行目录，即 `$TEST_WORKSPACE/traceability/<change_set_slug>/`

上游输入从 `$TEST_WORKSPACE/test_cases/`、`$TEST_WORKSPACE/clarification/`、`$TEST_WORKSPACE/metersphere/` 读取；本 skill 自己的产物写入 `$TRACEABILITY_RUN_DIR`。
...
| 项 | 适用 mode | 校验 | 失败提示 |
| --- | --- | --- | --- |
| `test_cases/final_cases.json` 存在 | **仅 traceability 标准模式**（smoke-test 不强制 final_cases，由 1.3.d 评估整体用例输入质量）| 文件可读、顶层 array 非空 | 先跑 test-case-generation 产出 final_cases；smoke-test 模式可改用 change-analysis 产出 supplementary cases |
| mapping sha 一致（仅当 mapping 存在时校验）| **仅 traceability 标准模式**（smoke-test 不写 MS 故跳过此项）| `metersphere/ms_case_mapping.json.source_cases_file.sha256 == sha256(test_cases/final_cases.json)` | 跑 `metersphere_helper.py refresh-mapping --diff-only` 查看差异，再 `--apply` 修复 |
...
| 项 | 校验 | 缺失时行为 |
| --- | --- | --- |
| `metersphere/ms_case_mapping.json` 存在 | 文件可读、顶层为 `{generated_at, source_cases_file, ms_project_id, entries}` 结构（v2 格式）| chat 输出**警告**：「metersphere/ms_case_mapping.json 不存在；Phase 6 writeback 会被跳过」。**不 STOP**。Phase 6.1.b 二次校验时整个 Phase 6 优雅 skip。|
| `metersphere/ms_plan_info.json` 存在 | 文件可读、含 `plan_id` 字段 | chat 输出**警告**：「metersphere/ms_plan_info.json 不存在；Phase 6 writeback 会被跳过；如需写 MS，请补跑 `metersphere-sync mode=sync` 完成测试计划创建后重跑本 skill」。**不 STOP**。Phase 6.1.b 二次校验时整个 Phase 6 优雅 skip。|
...
**步骤**：扫描公共工作区是否存在以下任一非空文件：

- `test_cases/final_cases.json` （上游 test-case-generation 产出）
- `change_supplementary_cases.json` （上游 change-analysis 产出，详见 [change-analysis PHASES §6](../change-analysis/PHASES.md)）
- `clarification/requirement_points.json` （上游 requirement-clarification 产出）

按下表设置 `input_quality`，并写入 `$TRACEABILITY_RUN_DIR/_input_quality.json`：
...
1. 公共工作区中存在上游产出文件（`clarification/clarified_requirements.json`）→ 读取作为需求理解基础，跳过 2.1
2. 如存在 `clarification/requirement_points.json` → 读取作为需求点清单
...
2. 用例文件中：
   - `final_cases.json` 以 `case_id` 作为用例主键；需求点关联优先用 `title` + `steps[].action` + `steps[].expected` + `preconditions` 的业务语义匹配 FP-N，`module` 仅作为分组和辅助上下文，不作为唯一关联依据
...
2. **单条消息**发 M 个 Task 调用：每个 Task 的 prompt 严格按 `agents/case-tracer.md` 模板填充。Agent 定义真源位于本 skill 的 `agents/case-tracer.md`：
   - 模块名 `{M}`
   - 该模块的 cases 子集
   - 该模块涉及的 diff 文件路径清单
   - 输出落盘路径：`$TRACEABILITY_RUN_DIR/forward_verification.{M}.json`
3. **每个 sub-agent 必须自校验**：用 `metersphere_helper.py validate-fv $TRACEABILITY_RUN_DIR/forward_verification.{M}.json` 校验通过才能返回；不通过 sub-agent 内部修复重试
4. 所有 sub-agent 完成后，主 agent 合并所有 `forward_verification.{M}.json` → `forward_verification.json`，再统一跑 4.6a 校验

**降级路径**（详见 [Recovery Cookbook §R-32](../commons/RECOVERY.md#9-r-32phase-32dispatch-sub-agent-失败d5)）：

| 失败场景 | R-code |
| --- | --- |
| Task 工具不可用 / 全部 sub-agent 启动失败 | [R-32-1](../commons/RECOVERY.md#r-32-1--task-工具不可用--全部-sub-agent-启动失败) |
| 单 sub-agent 多次校验失败 | [R-32-2](../commons/RECOVERY.md#r-32-2--单-sub-agent-持续校验失败) |
| sub-agent 超时 | [R-32-3](../commons/RECOVERY.md#r-32-3--sub-agent-超时) |
...
校验由 §4.6a `validate-fv` 强制执行（详见 [test-shared-tools/scripts/metersphere_helper.py](../test-shared-tools/scripts/metersphere_helper.py)）。校验失败 → STOP，模型必须修复 evidence 重落盘。
...
将所有用例的验证结果写入 `forward_verification.json`。完整 schema 在 `../commons/schemas/forward_verification.schema.json`，落盘后必须通过 `metersphere_helper.py validate-fv` 校验（见 4.6 之后的 4.6a）。
...
**上游优先（性能优化）**：如果工作目录已有 `api_contract_report.json`（用户已独立跑过 `api-contract-validation`）→ 跳过 agent 启动，直接在阶段 4 合并（节省一次 Task 启动开销）。否则启动共享 Agent。

**启动 Agent**：通过 Task 工具启动 `api-contract-validator` Agent（Agent 定义见 [test-shared-tools/agents/api-contract-validator.md](../test-shared-tools/agents/api-contract-validator.md)）。Codex 环境先读取该真源文件，默认主 Agent 内联执行；仅当用户明确要求并行/子 agent 时，使用内置 `worker` 并把定义全文嵌入 prompt。传入：
...
3. 从 `code_dir` 读取相关组件的样式定义代码片段：
   ...
   - 携带文件路径 + 行号
4. 通过 Task 工具启动 `ui-fidelity-checker` Agent（Agent 定义见 [test-shared-tools/agents/ui-fidelity-checker.md](../test-shared-tools/agents/ui-fidelity-checker.md)）。Codex 环境先读取该真源文件，默认主 Agent 内联执行；仅当用户明确要求并行/子 agent 时，使用内置 `worker` 并把定义全文嵌入 prompt。传入 Figma 数据 + 代码样式片段
...
对每个映射对判定方向：
   - **双向确认**：正向和反向都确认同一映射 → `trace_direction: "bidirectional"`，confidence 按 [CONVENTIONS.md 跨 Agent 共识规则](../commons/CONVENTIONS.md#跨-agent-共识规则) 计算：双方 confidence 均 >= 70 → `min(100, max(两者) + 20)`；任一方 < 60 → 不加成，取两者算术平均值；其余情况（一方 60-69）→ 不加成，取两者中较高值
...
1. 检查 `$TRACEABILITY_RUN_DIR/forward_verification.json` 是否存在且非空。
...
**`source` 字段是兜底版的硬约束标记**——schema 校验把它当作 evidence 缺失的唯一豁免凭证。**写每一条兜底记录时都不能漏**（详见 [R-46-1](../commons/RECOVERY.md#r-46-1--漏-source-字段d2) 自校验清单）。下游 metersphere-sync Phase 4.4 看到该字段时，回写评论追加"AI 回溯（降级判定，无用例级粒度）"。
...
python3 $SKILLS_ROOT/test-shared-tools/scripts/metersphere_helper.py \
  validate-fv $TRACEABILITY_RUN_DIR/forward_verification.json
```

- 校验通过（exit 0）→ 继续下一阶段（标准模式：4.7 → 5/6；smoke-test 模式：5S.1 → 5S.2）
- 校验失败（exit 2）→ stderr 是结构化 `{type: validation, errors: [{path, message}]}`，**stop**

**异常处理**（详见 [Recovery Cookbook](../commons/RECOVERY.md#2-r-vfvvalidate-fv-校验失败)）：

| 错误特征 | R-code |
| --- | --- |
| pass 缺 evidence | [R-VFV-1](../commons/RECOVERY.md#r-vfv-1--pass-缺-evidence) |
| code_location 文件不存在 | [R-VFV-2](../commons/RECOVERY.md#r-vfv-2--code_location-文件不存在) |
| pass + conf < 70 | [R-VFV-3](../commons/RECOVERY.md#r-vfv-3--pass--conf--70) |
| fail 缺 actual / evidence | [R-VFV-4](../commons/RECOVERY.md#r-vfv-4--fail-缺-actual-或-evidence) |
| pass + conf≥85 缺 considered_failure_modes | [R-VFV-5](../commons/RECOVERY.md#r-vfv-5--pass--conf85-缺-considered_failure_modes) |
| external_dependencies.types enum 越界 | [R-VFV-6](../commons/RECOVERY.md#r-vfv-6--external_dependenciestypes-enum-越界) |
| 4.6 兜底漏 source 字段 | [R-46-1](../commons/RECOVERY.md#r-46-1--漏-source-字段d2) |

**STOP 时的产物处置**（应用 P-CARRY-FORWARD pattern）：4.6a 失败时**前 4 个 phase 的产物仍可用**，chat 必须输出「已落盘清单 + 中断说明 + 三条修复路径」。详见 [R-VFV-STOP](../commons/RECOVERY.md#r-vfv-stop--46a-失败时的产物处置应用-p-carry-forward)。
...
对每条高 conf fail，逐条按交互式提问协议确认（不批量；Claude 可用 AskUserQuestion，Codex 用编号选项）：
...
**异常处理**（详见 [Recovery Cookbook](../commons/RECOVERY.md)）：

| 场景 | R-code |
| --- | --- |
| 用户回复不明确 / 不可解析 | [R-AQ-1](../commons/RECOVERY.md#r-aq-1--phase-47-用户回复不明确d1) |
| 选 B 后 schema 失败（缺 verification_logic 等） | [R-AQ-2](../commons/RECOVERY.md#r-aq-2--phase-47-选-b-后-schema-失败d1) |
...
**该提问必须按交互式提问协议发出**（Claude 可用 AskUserQuestion，Codex 用编号选项并等待用户回复），option 提供上述 3 个动作 + 第 4 个元操作「停止自循环（输出当前状态）」。

**「用户终止」判定标准**（替代旧的"无响应"模糊措辞）：
- 用户选择第 4 项「停止自循环」→ 立即退出，`exit_reason: "user_terminated"`
- 用户回复中明确包含"停止 / 取消 / 不修了 / 算了 / 就这样"等终止意图 → 同上
- 用户回复无法解析（如返回非选项内容且无明确意图）→ **不要静默退出**，再次按交互式提问协议澄清意图（最多 1 次）；澄清后仍不可解析才退出，`exit_reason: "user_terminated"` + 在 loop_metadata 标 `last_response_unparseable: true`
...
>
> **与 metersphere-sync mode=execute 的关系**：本 Phase 6 与 `metersphere-sync mode=execute` 共享同一 helper（`metersphere_helper.py writeback-from-fv`），是同一职责的两个调用入口（自动模式走本 Phase；手动模式用户可直接调 ms-sync）。共享/互斥规则详见 [`../commons/contracts/known-collisions.yaml`](../commons/contracts/known-collisions.yaml) 的 `forward_verification.enriched.json` 条目。

### 6.1 前置校验

#### 6.1.a 硬约束

1. **fv 完整性**：确认 `$TRACEABILITY_RUN_DIR/forward_verification.json` 存在、非空、且通过 4.6a `validate-fv` 校验。
...
2. **mapping 完整性**：检查 `$TEST_WORKSPACE/metersphere/ms_case_mapping.json`：
...
3. **plan_id 必须就位**：writeback-from-fv 需要 `plan_id` 入参，**不会创建 plan**。检查 `$TEST_WORKSPACE/metersphere/ms_plan_info.json` 是否存在：
...
> **历史变更**：旧版本写「通过 Skill 工具调用 `metersphere-sync` mode=execute」。**这条路径在实践中跑不通**——单会话只能调一个 skill，traceability 内部不能再 Skill() 调 metersphere-sync。
>
...
python3 $SKILLS_ROOT/test-shared-tools/scripts/metersphere_helper.py \
  writeback-from-fv \
  --plan-id <plan_id from metersphere/ms_plan_info.json> \
  --fv-path $TRACEABILITY_RUN_DIR/forward_verification.json \
  --mapping-path $TEST_WORKSPACE/metersphere/ms_case_mapping.json
...
6. **三个产物**（自动落盘到 `$TRACEABILITY_RUN_DIR/`）：
...
1. 确认 `$TRACEABILITY_RUN_DIR/ms_sync_report.json` 已生成且非空
2. 从 `summary.by_target_status` 提取 Pass/Prepare/Failure；从 fv 统计 inconclusive_reason 分组
3. **按情形选 chat 模板**（详见 [Recovery Cookbook](../commons/RECOVERY.md)）：

| 情形 | 模板 |
| --- | --- |
| 常态（Pass 或 Failure 占多数） | [R-P6-NORMAL](../commons/RECOVERY.md#r-p6-normal--phase-63-常态摘要模板) |
| `by_target_status.Pass == 0 && Failure == 0`（全 Prepare） | [R-P6-3](../commons/RECOVERY.md#r-p6-3--fv-全-inconclusive-警示摘要d7) |
| Phase 6 被 6.1.b 优雅 skip | [R-P6-1](../commons/RECOVERY.md#r-p6-1--missing-ms_case_mappingd10) 或 [R-P6-2](../commons/RECOVERY.md#r-p6-2--missing-ms_plan_infod3) |

### 6.4 失败处理

helper 调用 exit code != 0 → stderr 是结构化 JSON，按 `type` 走 [Recovery Cookbook](../commons/RECOVERY.md)：

| stderr.type | R-code |
| --- | --- |
| `precondition_failed` / `not_found`（mapping/plan_info 缺失） | [R-P6-1 / R-P6-2](../commons/RECOVERY.md#10-r-p6phase-6-整段优雅-skip应用-p-skip-phase) |
| `stale_mapping` | [R-PRE-2](../commons/RECOVERY.md#r-pre-2--mapping-sha-不一致硬阻断) |
| `validation`（fv 不合规） | [R-VFV-* + R-VFV-STOP](../commons/RECOVERY.md#2-r-vfvvalidate-fv-校验失败) |
| `api_error` 不可重试 | [R-WB-3](../commons/RECOVERY.md#r-wb-3--api_error-不可重试) |
| `network` 可重试 | [R-WB-4](../commons/RECOVERY.md#r-wb-4--network-持续失败) |
