...
2. **单条消息**发 M 个 Task 调用：每个 Task 的 prompt 严格按 `agents/case-tracer.md` 模板填充。Claude 注册入口 `agents/requirement-traceability/case-tracer.md` 是软链；真源位于本 skill 的 `agents/case-tracer.md`：
   - 模块名 `{M}`
   - 该模块的 cases 子集
   - 该模块涉及的 diff 文件路径清单
...
**启动 Agent**：通过 Task 工具启动 `api-contract-validator` Agent（真源见 [test-shared-tools/agents/api-contract-validator.md](../test-shared-tools/agents/api-contract-validator.md)，Claude 注册入口 `agents/api-contract-validator.md` 是软链）。Codex 环境先读取该真源文件，默认主 Agent 内联执行；仅当用户明确要求并行/子 agent 时，使用内置 `worker` 并把定义全文嵌入 prompt。传入：
...
3. 从 `code_dir` 读取相关组件的样式定义代码片段：
   ...
   - 携带文件路径 + 行号
4. 通过 Task 工具启动 `ui-fidelity-checker` Agent（真源见 [test-shared-tools/agents/ui-fidelity-checker.md](../test-shared-tools/agents/ui-fidelity-checker.md)，Claude 注册入口 `agents/ui-fidelity-checker.md` 是软链）。Codex 环境先读取该真源文件，默认主 Agent 内联执行；仅当用户明确要求并行/子 agent 时，使用内置 `worker` 并把定义全文嵌入 prompt。传入 Figma 数据 + 代码样式片段
