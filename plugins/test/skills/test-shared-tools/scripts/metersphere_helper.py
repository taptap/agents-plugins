...
环境变量（调用 MS API 的子命令必须设置；validate-fv 等纯本地命令不需要）:
...
_LOCAL_ONLY_COMMANDS = frozenset({'validate-fv'})


def _check_ms_credentials(cmd: str) -> None:
    """在 main() dispatch 前调用：要 MS API 的子命令缺凭据时 fail；纯本地命令跳过。"""
    if cmd in _LOCAL_ONLY_COMMANDS:
        return
    missing = [k for k in _REQUIRED_MS_ENV if not os.environ.get(k)]
    if missing:
