...
- 输出产物：`final_cases.json`（必须）、`test_cases.json`（review 前中间版本）、`tc_gen_review.md`、`review_summary.json`、`decomposition.md`、`context_summary.md`（仅 >= 3 模块时生成）、`sufficiency_assessment.json`（充分性评估）
- 失败门控：需求正文不可读时停止生成；需求充分性不足时暂停请求用户补充或标注风险；所有用例必须严格基于需求，不补充未提及功能
- 执行步骤：`init → understand（含充分性门控）→ decompose → generate → review → confirm → output`
...
- 支持通过 `re_entry_phase` 参数从指定阶段重入（合法值：understand / decompose / generate / review），配合 `requirement_change_summary` 实现需求变更后的增量更新，仅重新生成受影响模块的用例
