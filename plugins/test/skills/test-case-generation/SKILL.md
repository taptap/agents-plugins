...
按以下 8 个阶段顺序执行，各阶段详细操作见 [PHASES.md](PHASES.md)。
...
JSON 字段定义见 [CONVENTIONS.md 用例 JSON 格式](../commons/CONVENTIONS.md#用例-json-格式)（含完整字段表和约束）。顶层必须是 JSON 数组，每条用例通过 `module` 字段标识归属模块。补充要求：
...
- [ ] `tc_gen_review.md` — 非空，包含冗余对评审结果
- [ ] `review_summary.json` — 非空，包含评审统计
- [ ] `decomposition.md` — 非空，包含功能模块拆解清单
- [ ] `context_summary.md` — 可选，仅模块 >= 3 个时生成
- [ ] `clarifications.json` — 可选，仅阶段 3.5 扫到 ≥ 1 条歧义时生成；存在时 `final_cases.json` 中带 `[待确认]` 前缀的 step 数应与 pending 项数量基本一致
