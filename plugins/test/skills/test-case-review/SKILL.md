...
description: >
  ...
  输出 review_result.json + review_summary.md + 可选 review_supplementary_cases.json。
...
- 输出产物：`review_result.json`（评审 verdict 对象，LLM 友好的非严格 JSON，不绑 schema 约束）、`review_summary.md`、可选 `review_supplementary_cases.json`（受 ../commons/contracts/testcase.schema.json 约束）、`requirement_points.md`
...
| 5. summary | 统计汇总 + 补充用例生成 | `review_summary.md`、`review_supplementary_cases.json` |
...
| `review_supplementary_cases.json` | summary | 补充用例（可选） |
...
- [ ] `review_supplementary_cases.json` — 可选，仅当存在覆盖缺口时生成
