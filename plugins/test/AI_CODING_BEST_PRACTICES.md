...
```bash
export TEST_WORKSPACE=requirement_<stable_id>
mkdir -p $TEST_WORKSPACE
```

例如 `requirement_TAP-12345` 或 `requirement_<飞书文档 token>`。所有 skill 的产物会按 `clarification/`、`test_cases/`、`metersphere/`、`traceability/` 子目录汇聚，跨 skill 自动消费上游产出。`requirement_*/` 已在 `.gitignore` 里。
...
确认 `$TEST_WORKSPACE/test_cases/final_cases.json` 生成 OK 后：

```
用 test:metersphere-sync 把 $TEST_WORKSPACE/test_cases/final_cases.json 同步到 MS，
...
```
按 $TEST_WORKSPACE/clarification/clarified_requirements.json 里的任务清单出实现计划。
