...
- **v0.0.14** - 两个改动合并发布。**(1) ui-fidelity-check skill 删除**：底层 `agents/ui-fidelity-checker.md` 保留并简化（去 Browser MCP，纯静态对比代码样式 vs Figma 数据）；UI 还原度检查现仅由 `requirement-traceability §3.4` 触发（design_link + code_dir 都有时启动，confidence 上限 60，跨端可用）。**(2) api-contract-validation skill 保留 + 抽 agent 共享**：新建 `agents/api-contract-validator.md`（无副作用纯计算单元），独立 skill 和 traceability §3.2.5 都通过 Task 工具启动同一 agent，消除"轻量内置 + 上游优先"二选一的混乱；上游优先仅作为性能优化（如 workspace 已有 api_contract_report.json 则跳过 agent 启动）。**(3) requirement-traceability 输出规范 A/B/D 强制 + C 自检**：新增 §3.2.0a evidence completeness 契约——A 数据流闭合、B 跨边界自然记录、D 失败模式路径驱动这三条由 `metersphere_helper.py validate-fv` 机械校验；C 逐 expected 对应是模型自检建议（不入 validator——机械校验需要交叉读 case 文件 + 关键词匹配，易被同义词绕过，强行实装反而让用户产生"validator 已经验过"的虚假安全感）。`tests/validate.sh` 新增 Check N+2 锁定 A/B/D 行为。同时影响：qa-workflow 步骤号重排（删除 ui-fidelity-check 步骤后 #6→ api-contract，#7→ traceability，#8→ user_gate，依此类推）；README / PIPELINES / TRACEABILITY_PROTOCOL / known-collisions.yaml 同步清理
- **v0.0.13** - README ↔ AI_CODING_BEST_PRACTICES 去重，明确职责边界。README 退回 catalog 角色（"插件有什么 / skill 怎么选 / 在哪个目录 / 怎么配 / 改了什么"），完整工作流 SOP 全部由 BP 承载。具体改动：(1) 删「场景二：AI coding 工作流编排」的 6 步流程图，改成单行外链到 BP；(2) 删「链路 A：功能测试全流程」的数据流图，保留一句概述 + 外链；(3) 精简「链路 D：需求回溯增强」，去掉与 BP 阶段 4 重复的叙事，保留 traceability + ui-fidelity 组合的数据流契约；(4) 修「环境变量」段 MS_* 4 行失实描述（原写「有默认值」实际无），改成飞书配置链接 + 懒人路径说明
- **v0.0.12** - 修复 `metersphere-sync` 配置的失实描述：`SKILL.md` 和 `AI_CODING_BEST_PRACTICES.md` 原本说 MS 凭据「零配置 / 已内置」，但 `metersphere_helper.py` 实际从 `.env` 读取，仅 `MS_DEFAULT_MAINTAINER` / `MS_DEFAULT_STAGE` 有内置默认值。两份文档改为描述真实的「懒人路径」UX：不用提前手动配，首次跑报 `missing required environment variables` 时，把飞书配置块（`https://xd.feishu.cn/wiki/K4Cxw8HE5itR16kFFYicSctAnrc`）整段贴给 AI，让它写入 `.env` 即可。`SKILL.md` 环境变量表从 6 行扩到 11 行，补齐缺失的 `MS_WORKSPACE_ID` 和 4 个 `MS_FIELD_ID_*`，默认值列改正；Troubleshooting 中 MS 401/403 / 404 条目也修正（原来归因「默认凭据被覆盖」是不可能的，因为根本没有默认凭据）
- **v0.0.11** - `AI_CODING_BEST_PRACTICES.md` 工程师视角重写：新增「按问题查」索引 + §0 Quickstart（5 步起飞）+ 各阶段「复制即用 prompt 模板」+ Phase 3 plan-mode 三段式 walkthrough（设计/评审/实施 + 评审三件套表）+ 「反模式」「Troubleshooting」「术语小词典」三个新通用章节；阶段 3 移除 `change-analysis` 引用（改放进选读）；进一步阅读分必读/选读；mermaid 图把「独立 AI review」并入阶段 5 表述；统一 callout 风格（💡 原则 / ⚠️ 坑 / ✅ 产出 / 📥 输入）。**团队内部内容（iOS TAP-6841255319 案例 / 裸需求场景对比 / 角色分工 RACI / 内部联系人）迁移至飞书 wiki**（`https://xd.feishu.cn/wiki/PgRLwgQj2iUkiGk0Xm8ctGMWnTb`），repo 仅留单行指针 — 因为这些内容带工单 ID/PR 编号/特定组织结构，对开源 marketplace 受众是噪音
- **v0.0.10** - requirement-review/requirement-clarification 单 Agent 强推理重构（删除 user/functional/exception-perspective 三个子代理）：4.1.7 反 collapse 枚举归一化 + 4.1.8 多变体一致性约束（防类继承级联导致 UI 隐式扩散）、`rr_summary.json` 新增 `confidence` 字段（0-100，schema required）+ verdict 优先级链硬规则（命中即停，消除 OR 多档同时命中歧义）。`test-case-review` / `requirement-traceability` / `change-analysis` TEMPLATES 改用表格 + `[有效/无效]` `[已修复/无需修复]` checkbox 反馈结构；`requirement-traceability` 缺陷清单加 `---` 分隔；`change-analysis` §6 加 cross-validation 综合结论 checkbox。新增 `CONVENTIONS.md`「飞书文档渲染规范」段（取代旧 "no markdown table" 规则；表格/bullet/checkbox 选用规范）；`_shared/REQUIREMENT_DIMENSIONS.md` 加 severity 阻断/关注（P0/P1 alias）+ verdict↔confidence 映射；`contracts/rr-summary.schema.json` `confidence` / `blocking_issues` 改 required；`tests/check-schemas.sh` 加越界/缺失拒收用例。`requirement-review/PHASES.md` 4.0 模式说明改中性命名（设计稿评审模式 / 描述评审模式，去掉旧版「降级」贬义；review 不引入 clarification 的探索式模式）
