...
- **v0.0.17** - code review fixup：requirement-review verdict 硬规则改为优先级链（命中即停，消除 OR 多档同时命中的歧义）；同步 `_shared/REQUIREMENT_DIMENSIONS.md` 对应表与 `requirement-review/PHASES.md` 4.0 模式说明（review 仅 3 模式，不含 clarification 的探索式）；补 v0.0.10-v0.0.16 changelog
- **v0.0.10-v0.0.16** - 累积版本：详见各 plugins/test 提交，主要包括 `qa-workflow` 编排、`metersphere-sync` skill、Codex `interface` 段补全、跨仓库契约桥（`contract-bridge-check.py` + 5 个 JSON schema）、`codex_agent.py` 独立 Codex 代理、change-analysis cross-validation、需求侧 skill 量化置信度统一、ai-case 模型同步与 contract collision 治理、`tests/validate.sh` 9 类检查、CONVENTIONS 飞书文档渲染规范、requirement-review/clarification 单 Agent 强推理重构 + 飞书报告 checkbox 反馈闭环
