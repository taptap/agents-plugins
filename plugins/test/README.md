...
- **v0.0.10** - 一次大版本，6 块改动合并发布：
  ...
  9. **`test-case-generation/PHASES.md §2.4` 单 Agent 多维度强推理**（与 §1 单 Agent 重构配套）：functional / exception / user 三维度在主 Agent 上下文内串行执行；移除全部对已删除 `agents/requirement-understanding/...` 的 `Read` / `Task` 调用。`commons/AGENT_PROTOCOL.md` 目录树和命名约定同步删除 `requirement-understanding/`。`metersphere-sync/PHASES.md §4.0` 冒烟前置改为处理全部 5 个 verdict 枚举值（`fail` / `fail-with-degraded-input` / `inconclusive` / `pass` / `pass-with-warnings`），其中 `fail-with-degraded-input` 和 `inconclusive` 与 `fail` 同样触发 Prepare 降级。
