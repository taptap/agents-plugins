...
| Skill | 功能 |
|-------|------|
| **test-shared-tools** | 共享脚本集合（飞书文档获取、GitLab/GitHub MR/PR 分析、MeterSphere 用例同步） |
...
| 写新用例 / 需求歧义补充用例（**需求驱动**）| test-case-generation | 输入需求/澄清结果，输出 `test_cases/final_cases.json`（含可选 supplementary） |
...
| 需求实现后验证代码是否正确 | requirement-traceability | 双通道追溯（正向通道内嵌用例中介验证，消费上游 `test_cases/final_cases.json`） |
...
| 需求 | 调用 skill | 输入 | 输出 |
|------|-----------|------|------|
| 需求澄清 | requirement-clarification | 需求链接/文档 | `clarification/clarified_requirements.json` |
| 用例生成 | test-case-generation | 需求链接/文档 | `test_cases/final_cases.json` |
...
> 链路 B 可接收链路 A 的 `clarification/requirement_points.json` 作为可选输入，用于指导测试覆盖重点（优先为 P0/P1 功能点对应的代码模块生成测试）。
...
```
test_cases/final_cases.json (链路 A) + 代码实现 + design_link/code_dir + (可选) backend_changes/openapi_spec
    ↓
requirement-traceability（双通道回溯 + §3.4 调用 ui-fidelity-checker agent + §3.2.5 调用 api-contract-validator agent）
    ...
    ↓ traceability/<change_set_slug>/ui_fidelity_report.json（design_link + code_dir 都有时）
    ↓ api_contract_report.json（API 相关变更触发时；如已有上游产出则跳过 agent 启动）
    ↓ traceability/<change_set_slug>/traceability_coverage_report.json（含正向验证率 + UI 还原度 + API 契约一致性）
```
...
test-shared-tools 脚本依赖以下环境变量（按需配置）：
...
- **v0.0.10** - 一次大版本，多块改动合并发布：
  0. **飞书报告 checkbox 语义修复**：`change-analysis/PHASES.md` 取消「更新清单标记为 `[x]`」强指令——改为保留 `- [ ]`，让 QA / 评审会拥有勾选权（之前 AI 自动勾上让读者误读为"已测试覆盖"）；`change-analysis/PHASES.md §6.4 + TEMPLATES.md §6.4` 加禁令 + 反例：交叉验证综合结论的一级 bullet 必须用 `- **{发现}**: {描述}` 粗体（无 checkbox），仅嵌套层用二选一空 checkbox（之前 AI 把"整体置信度"、"联合建议"等纯结论描述也写成 `- [x]`）；`requirement-review/PHASES.md §3` 强调每个父项必须嵌套 `- [ ] 有效问题 / - [ ] 无效问题` 二选一空 checkbox（之前 AI 只写父项，导致评审会无勾选位）。
  ...
  7. **`shared-tools` → `test-shared-tools` 重命名**：原名过于通用，下游仓库同时安装其他「shared tools」类 skill 时存在冲突风险。Skill 名 + 目录改为 `test-shared-tools`，所有 `$SKILLS_ROOT/shared-tools/...` 引用全量同步（`README.md` / `CONTRACT_SPEC.md` / `AI_CODING_BEST_PRACTICES.md` / 全部消费 skill 的 `SKILL.md`/`PHASES.md`/`contract.yaml` / `case-tracer.md` / `codex-change-analyzer.md` / `commons/TRACEABILITY_PROTOCOL.md` / `tests/validate.sh` / `.pre-commit-config.yaml`）。脚本内部逻辑不变。
  8. **`test-case-generation/contract.yaml` 输入输出修订**：`clarified_requirements` 和 `requirement_points` 从 `optional` 移到 `required`（两者成对出现，任一存在即可启动 session）；新增 `final_cases.json` 输出条目（output 阶段的最终用例集，对外可入库）；`test_cases.json` 标 `optional`，描述改为 review 前快照（已被 `final_cases.json` 覆盖）。
  9. **`test-case-generation/PHASES.md §2.4` 单 Agent 多维度强推理**（与 §1 单 Agent 重构配套）：functional / exception / user 三维度在主 Agent 上下文内串行执行；移除全部对已删除 `agents/requirement-understanding/...` 的 `Read` / `Task` 调用。`commons/AGENT_PROTOCOL.md` 目录树和命名约定同步删除 `requirement-understanding/`。`metersphere-sync/PHASES.md §4.0` 冒烟前置改为处理全部 5 个 verdict 枚举值（`fail` / `fail-with-degraded-input` / `inconclusive` / `pass` / `pass-with-degraded-input`），其中 `fail-with-degraded-input` 和 `inconclusive` 与 `fail` 同样触发 Prepare 降级。
  10. **Smoke-test source 8 — low-input-quality 补偿搜索**：`requirement-traceability/PHASES.md §5S.1` 新增 SEARCH-A（new data-source coverage）/ SEARCH-B（time-field boundary）/ SEARCH-C（batch-operation bounds），仅在 `input_quality == "low"` 时触发；GameJam case 实测 recall 0/3 → 3/3。`contracts/defect-list.schema.json` 新增 `evidence.search_id`（pattern `^S[ABC]-\d+$`）+ `if/then` 强制 `search_id` 在 `evidence.source ∈ {search-a/b/c}` 时必填（6 fixtures 全绿）。`requirement-traceability/TEMPLATES.md` 新增缺陷命名硬约束表（`title→name`/`desc→description`/`expected→expected_result`/`actual→actual_result`），删除冗余「(注意是 X 不是 Y)」尾注。
  11. **`skills/_shared/` → `skills/commons/` 重命名**：原名与同级 skill `test-shared-tools/` 视觉混淆，且「shared」语义不清（实际承载跨 skill 共享框架/协议/规范/schema 的「杂物抽屉」）。`git mv` 整目录，22 处 skill 文件 + `CONVENTIONS.md` + `tests/validate.sh` 路径过滤 + `metersphere_helper.py` 路径常量同步更新；本 README 目录树同步至 `skills/commons/`。脚本逻辑不变，仅路径/名称更新。
  12. **Agent prompt 真源就近 + commons 提升（跨 runtime 工效）**：reusable Agent prompt 定义迁移到各 skill 的 `agents/` 目录作为单一真源，移除 `plugins/test/agents/**` 伪注册软链层；`CONVENTIONS.md` 和 `contracts/` 真源迁移到 `skills/commons/`，根目录保留兼容软链确保 Codex skill-only 安装也能解析共享规范与 schema；明确跨 runtime 协议——Claude Code 和 Codex 都读取 skill 内 Agent 定义执行，Claude 用通用 Task agent，Codex 默认内联或在明确委托时用内置 `default` / `explorer` / `worker`。`requirement-traceability` / `test-case-generation` / `api-contract-validation` / 共享 Agent 协议引用全部更新到 skill 内路径。
  13. **iOS PR #312 回迁 — requirement-scoped workspace + cases MCP server + codex_agent 加固**：新增 `requirement_<stable_id>/` 公共需求工作区与 `manifest.json`，统一 `clarification/` / `test_cases/` / `metersphere/` / `traceability/` 产物落点；引入跨 Claude / Codex 的单题 `InteractiveQuestion` 协议和 `clarification_gate.json` 门禁脚本；新增独立 stdio `cases_mcp_server.py`，让普通仓库也能使用 `save_test_cases` 的路径围栏、schema 校验、`case_id` 自动分配和 `module` 规范化；加固 `codex_agent.py` 的 `shell=False` 执行 + helper 白名单 + 敏感 `.env` 读阻断；test-case-generation / metersphere-sync / requirement-traceability 全部对齐共享工作区（最终用例落 `test_cases/`、MS mapping/import/plan 落 `metersphere/`、回溯运行落 `traceability/<change_set_slug>/`）；contract metadata 扩展 optional outputs / dynamic filename patterns / schema pointers / produced-when conditions / consumer indexes。
- **v0.0.9** - 飞书报告可读性优化：requirement-review 重构 `TEMPLATES.md`（10 处可读性优化 + 5 处飞书 import 兼容性修复），test-case-review / change-analysis / requirement-traceability 新建 `TEMPLATES.md`（覆盖 review_summary.md / multi-doc / smoke-test report.md）；统一 4 工作流报告 11 条约定（无 H1、无表格、无 emoji、无 ASCII、无星级、统一中文方括号 `[通过]/[已覆盖]/[实证]/[P0]` 等）。修复 requirement-review `rr_summary.json` 写出非法 `review_mode` 值。test 插件全量评审 + CI 基线加固：`tests/validate.sh` 5 → 9 类检查（SKILL frontmatter / handoffs / subagent_type / references / contract collision），新增 `contracts/known-collisions.yaml` 白名单；解决 8 处 contract 输出冲突（`risk_assessment.json` → `bug_risk_assessment.json`、`test_execution_report.json` 拆 unit/integration、`supplementary_cases.json` 按上游 skill 拆 change/review）；删除 3 个零调用 agent (`forward-tracer`/`reverse-tracer`/`failure-classifier`) + 13 处死引用清理；test-shared-tools / feedback `requirements.txt` 与 env_vars 诚实化；5 个 helper 脚本 `-h/--help` UX 一致化；schema 校验从 1 → 5（testcase / ca-summary / defect-list / rr-summary / smoke-test-report）；同步 PIPELINES / README / TEMPLATES / contract.yaml 中 contract rename 后残留的旧文件名引用。8 维 skill 评审收尾（阶段 1-3）：3 个 contract.yaml 给 5 个 `*_cases.json` 输出显式绑 `testcase.schema.json`；tcg/ca SKILL description 加"需求驱动 / 变更驱动"区分 + ca↔trace 互引 SKIP；test-case-review SKILL 注明 `review_result.json` 为非严格 JSON（verdict 对象、不绑 schema）；从 ai-case Pydantic 同步 `testcase.schema.json` — `Step.expected` 改必填（允许空串），`test_method` 在 CONVENTIONS.md 同步改可选；trace PHASES Phase 6 / PIPELINES.md 加交叉引用到 known-collisions.yaml，澄清 Phase 6 writeback 与 ms-sync mode=execute 是"同 helper、双入口"；qa-workflow 接受 `re_entry_phase` + `requirement_change_summary`，支持需求变更后增量重跑透传 tcg；README 选用指引补 tcg/ca 驱动条件区分、去掉过期 "v0.0.10+" 版本门；requirement-clarification SKILL 注明 `output/*.json` 是预制格式样例非运行时产出；README + AGENT_PROTOCOL.md 说明 `agents/` 下 .md 无 frontmatter 不会被 loader 自动注册（仅由 skill 通过 Task tool 显式调用）
...
- 从 skills-hub 迁移 6 个 QA 工作流 Skill + test-shared-tools
